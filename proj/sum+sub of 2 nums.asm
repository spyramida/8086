...
START:      
    PRINT_STR MSG
    MOV CX, 02H				; 3 επαναλήψεις
DIGITS:
    CALL DEC_KEYB			; Διάβασμα των 3 ψηφίων
	...
	MOV CX,AX
	JMP WAIT_FOR_ENTER			; και μετά τοποθέτηση στον CX
ARNHTIKOS:
    SUB BX,AX
    MOV CX,BX
...
PRINTING:	
    ...
    CMP CX,BX
    JE  NEGATIVE_PRINT
    PRINT_STR SUBI ; Εκτύπωση πολλαπλασιασμού  
    MOV DX,CX
    CALL PRINT_HEX 
...
NEGATIVE_PRINT:
    PRINT_STR SUBINEG ; Εκτύπωση πολλαπλασιασμού  
    MOV DX,CX
    CALL PRINT_HEX 
    PRINT_STR NEWLINE		; Νέα γραμμή κι επανάληψη
    JMP START
...
READSTART: READ
	...
	JG NEXT2
	MOV AH,0
	SUB AX,30H				; Μετατροπή από ASCII χαρακτήρα σε δεκαεξαδικό ψηφίο
	RET          
NEXT2:    					; Αν είναι 0-9
    MOV AH,0
